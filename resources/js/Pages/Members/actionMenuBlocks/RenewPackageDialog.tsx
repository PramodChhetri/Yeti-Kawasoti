...
const RenewPackageDialog = ({
    ...
    const membershipPackage = packages.find(
        (p) => p.id === member.membership_package_id
    );

    const paidRef = useRef<HTMLInputElement>(null);

    // Using useForm for form management
    const { data, setData, reset, errors, post, processing, clearErrors } =
        useForm({
            selected_months: 1,
            extra_discount: 0,
            paid_amount: 0,
    ...
    useEffect(() => {
        if (membershipPackage) {
            ...
            if (data.selected_months === 3) {
                calculatedDiscount =
                    Number(membershipPackage.discount_quarterly) || 0;
            } else if (data.selected_months === 6) {
                calculatedDiscount =
                    Number(membershipPackage.discount_half_yearly) || 0;
            } else if (data.selected_months === 12) {
                calculatedDiscount =
                    Number(membershipPackage.discount_yearly) || 0;
            }
                    ...
                    <Card>
                        ...
                        <CardContent>
                            ...
                            <div className="flex justify-between">
                                ...
                                <strong>
                                    {
                                        member.payment_expiry_date as unknown as string
                                    }
                                </strong>
                            </div>
                            <hr className="my-2" />
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <InputError
                                            message={errors.selected_months}
                                        />
                                    </div>
